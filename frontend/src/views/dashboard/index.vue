    ...
    <el-row :gutter="20">
      ...
      <el-col :span="8">
        <el-card class="box-card">
          <template #header>
            <div class="card-header"><span>Ongoing Tasks</span><span class="task-count">Total: {{ dDataD.ongoingTasks.length }}</span></div></template>
          <el-scrollbar height="900px"><div v-for="task in dDataD.ongoingTasks" :key="task.taskId" class="task-item"><div class="task-title">{{ task.title }}</div>
      ...
      <el-col :span="8">
        <el-card class="box-card"><template #header><div class="card-header"><span>Completed Tasks</span><span class="task-count">Total: {{ dDataD.completedTasks.length }}</span></div></template><el-scrollbar height="900px"><div v-for="task in dDataD.completedTasks" :key="task.taskId" class="task-item"><div class="task-title">{{ task.title }}</div><div class="task-info"><el-tag size="small" type="success">Completed</el-tag>
